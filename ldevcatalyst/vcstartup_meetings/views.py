...
def meeting_request_list(request):
    ...
    for sent_request in sent_requests_q:
        if sent_request.receiver.user_role in [6, 8]:
            receiver_model = StartUp if sent_request.receiver.user_role == 6 else VC
            receiver_instance = receiver_model.objects.get(user_id=sent_request.receiver.id)
            temp = {
                'pk': sent_request.id,
                'receiver': sent_request.receiver,
                'receiver_name': receiver_instance.company_name if sent_request.receiver.user_role == 6 else receiver_instance.name,
    ...
    for received_request in received_requests_q:
        if received_request.sender.user_role in [6, 8]:
            sender_model = StartUp if received_request.sender.user_role == 6 else VC
            sender_instance = sender_model.objects.get(user_id=received_request.sender.id)
            temp = {
                'pk': received_request.id,
                'sender': received_request.sender,
                'sender_name': sender_instance.company_name if received_request.sender.user_role == 6 else sender_instance.name,
                'sender_id': sender_instance.id,
...
def meeting_request_detail(request, pk):
    ...
    return render(request, 'vcstartup_connect/meeting_request_detail.html', context)

@login_required
def meeting_request_create(request, receiver_id):
    receiver = get_object_or_404(User, pk=receiver_id)
    if receiver.user_role not in [6, 8]:
...
def meeting_request_update(request, pk):
    meeting_request = get_object_or_404(MeetingRequest, pk=pk)
    if meeting_request.sender != request.user:
        return redirect('meeting_request_list')
